...
| 中文关键词检索 | Python 分词 + PostgreSQL `tsvector` + GIN | 已确定，分词效果需验证 |
| 向量检索 | pgvector 精确余弦检索 | MVP 已确定 |
| 结果融合 | 倒数排名融合（RRF） | MVP 默认基线，效果对比与调参放到第二版 |
| 重排模型 | MVP 不使用 | 检索评测不达标时再引入 |
| 大语言模型 | 厂商无关的 OpenAI-compatible HTTP 接口 | 接口已确定，具体模型待凭据与样例验证 |
| Embedding 模型 | 通过 `Embedder` 接口接入 | 具体模型和维度待检索 truth test 后冻结 |
...
PostgreSQL 的 GIN 只解决索引访问，不自动解决中文分词。MVP 在 Python 应用侧完成中文分词，再生成供 `tsvector` 使用的规范化词串。

分词规则：

- 使用 Python 中文分词库处理自然语言；
- 单独提取并保留 NAS 型号、错误码、版本号、命令、协议名和英文缩写；
- NAS 领域词维护在项目词典中；
- 原文始终保留，分词结果只用于检索；
- 使用固定查询集验证分词是否破坏 `RAID5`、`S.M.A.R.T.`、`SMB`、错误码等关键标识。

MVP 不使用 `pg_jieba`，避免增加 PostgreSQL 原生扩展、镜像构建和版本兼容成本。
...
- 产品文档与历史案例的 Recall@K；
- NAS 型号、错误码和中英文混合查询表现；
- 向量维度与数据库占用；
- 单批嵌入延迟、费用和稳定性。

大语言模型至少验证：严格 JSON 输出成功率、引用约束、知识不足时拒绝确定性回答、高风险操作拦截和中文回复质量。

Embedding 模型或向量维度变更时创建新的知识索引版本，完成全量重嵌入后再切换，不能在同一向量列中混用不同模型或维度。
...
第一版的 `MockMailboxAdapter` 主动扫描本地模拟邮箱，不由业务接口直接提交邮件。
...
1. 使用小规模固定 NAS 查询集比较可用 Embedding 模型，确认基本 Recall@K 后冻结模型和向量维度。Embedding 变化会触发全量重嵌入，因此必须在建库前验证。
2. 验证大语言模型能够返回合法 JSON，引用本次真实检索片段，并在知识不足或高风险时遵守降级和危险操作拦截规则。
3. 对中文分词做最小冒烟验证，确保 NAS 型号、错误码、协议和命令不会被破坏；第一版不比较多个分词方案。
