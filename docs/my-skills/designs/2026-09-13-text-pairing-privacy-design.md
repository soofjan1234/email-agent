...
已冻结：产品语言为英文；Embedding 为 `text-embedding-3-small` / 1536；LLM 为 `gpt-4o-mini`。2026-09-13 第 3–5 条离线验证已通过，记录见 `evals/text-v1/freeze.json`。6–8 条仍依赖 Checkpointer 与业务副作用，本设计不覆盖。
...
| B. 先起 PostgreSQL 只测 `tsvector` | 第 3 条更接近上线检索 | 为冒烟引入数据库 | 维度已冻结但业务表未建，过早 | 不采用 |
